...
def _setup_serial(pipe):
    """Configure serial port for C2 communication.

    The C2 requires non-blocking reads (timeout=0). Any non-zero timeout
    sets VTIME in termios, which causes this radio to reset. The driver
    implements its own timeout loop in _read_frame instead.

    The serialtrace diagnostic wrapper warns when reads are issued with
    insufficient timeout for the baud rate. Since we use non-blocking
    reads with our own polling, these warnings are false positives.
    """
    pipe.timeout = 0
    warnings.filterwarnings('ignore', message='Read of .* bytes requires',
                            module=r'.*serialtrace')


def _read_frame(pipe, timeout=5.0):
    """Read a complete STX...ETX frame from the serial port.

    The frame uses DLE byte-stuffing, so we must track whether each
    byte is escaped to correctly identify the real ETX terminator.
    """
    data = bytearray()
    in_escape = False
    start = time.time()
...
def _send_recv(radio, payload):
    ...
    for attempt in range(3):
        LOG.debug("TX (%d bytes, attempt %d): %s",
                  len(frame), attempt + 1, frame.hex())
        try:
            pipe.write(frame)
            pipe.flush()
        except Exception as e:
        ...
        if resp_frame:
            LOG.debug("RX (%d bytes)", len(resp_frame))
        ...
        try:
            ...
            if avail:
                raw = pipe.read(avail)
                LOG.debug("Discarding %d bytes: %s", len(raw), raw.hex())
        except Exception:
            pass
...
def _read_block(radio, seg, block, sub):
    """Read a data block from the radio.

    Returns just the data portion (no header bytes).
    """
    payload = bytes([0x01, seg, block, sub])
    resp = _send_recv(radio, payload)
    # Response format: [resp_type, seg, block, sub, ...data...]
    if len(resp) < 4 or resp[1:4] != payload[1:4]:
        raise errors.RadioError(
            "Unexpected response header: got %s, expected seg=%02x "
            "blk=%02x sub=%02x" % (resp[:4].hex(), seg, block, sub))
    return resp[4:]


def _write_block(radio, seg, block, sub, data):
    """Write a data block to the radio."""
    payload = bytes([0x01, seg, block, sub]) + data
    frame = _make_frame(payload)
    LOG.debug("TX write (%d bytes payload, %d bytes frame)",
    ...
    if not resp_frame:
        ...
        raise errors.RadioError(
            "No response to write (seg=%02x blk=%02x sub=%02x, %d bytes)" %
            (seg, block, sub, len(data)))

    resp = _parse_frame(resp_frame)
    if len(resp) < 4 or resp[1:4] != payload[1:4]:
        raise errors.RadioError(
            "Unexpected write response: got %s, expected seg=%02x "
            "blk=%02x sub=%02x" % (resp[:4].hex(), seg, block, sub))
    return resp
...
def do_download(radio):
    ...
    # 1. Read ident
    data = _read_block(radio, SEG_IDENT, 0x00, 0x00)
    ...
    # 2. Read settings
    data = _read_block(radio, SEG_READ, BLK_SETTINGS, 0x00)
    mmap_data[offset:offset + len(data)] = data
    offset += len(data)
    status.cur = offset
    radio.status_fn(status)
    ...
    for i in range(NUM_CHANNELS):
        data = _read_block(radio, SEG_READ, BLK_CHANNELS, i)
        mmap_data[offset:offset + len(data)] = data
        offset += len(data)
        status.cur = offset
        if i % 16 == 0:
    ...
    # 4. Read extra blocks
    for i in range(8):
        data = _read_block(radio, SEG_READ, BLK_EXTRA, i)
        mmap_data[offset:offset + len(data)] = data
        offset += len(data)

    # 5. Read final block
    data = _read_block(radio, SEG_READ, BLK_FINAL, 0x00)
    mmap_data[offset:offset + len(data)] = data
    offset += len(data)
...
def do_upload(radio):
    ...
    _read_block(radio, SEG_IDENT, 0x00, 0x00)

    # 2. Read current settings (verify communication)
    _read_block(radio, SEG_READ, BLK_SETTINGS, 0x00)

    # 3. Write settings
    settings_data = mmap[SETTINGS_OFFSET:SETTINGS_OFFSET + SETTINGS_SIZE]
    _write_block(radio, SEG_WRITE, BLK_SETTINGS, 0x00, settings_data)
    _read_block(radio, SEG_VERIFY, BLK_SETTINGS, 0x00)
    ...
    for i in range(NUM_CHANNELS):
        ...
        _write_block(radio, SEG_WRITE, BLK_CHANNELS, i, ch_data)
        _read_block(radio, SEG_VERIFY, BLK_CHANNELS, i)
        status.cur = ch_offset + CHANNEL_SIZE
        if i % 16 == 0:
            radio.status_fn(status)

    # 5. Write extra blocks (8 blocks of 32 bytes each)
    extra_offset = CHANNELS_OFFSET + NUM_CHANNELS * CHANNEL_SIZE
    for i in range(8):
        blk_start = extra_offset + i * 32
        blk_data = mmap[blk_start:blk_start + 32]
        _write_block(radio, SEG_WRITE, BLK_EXTRA, i, blk_data)
        _read_block(radio, SEG_VERIFY, BLK_EXTRA, i)

    # 6. Write final block (448 bytes, matching CPS protocol)
    final_offset = extra_offset + 8 * 32
    final_data = mmap[final_offset:final_offset + 448]
    _write_block(radio, SEG_WRITE, BLK_FINAL, 0x00, final_data)
    _read_block(radio, SEG_VERIFY, BLK_FINAL, 0x00)

    # 7. Finalize
    _read_block(radio, SEG_FINALIZE, 0x00, 0x00)
...
class RetevisC2(chirp_common.CloneModeRadio):
    ...
    def _apply_gmrs_limits(self, mem):
        """Apply GMRS frequency-based restrictions (implied modes).

        Interstitial channels (467.5625-467.7125 MHz) are forced to
        NFM and low power. Non-GMRS TX frequencies are allowed but
        the radio itself inhibits transmission; this is reported via
        ValidationWarning rather than forced duplex=off.
        """
